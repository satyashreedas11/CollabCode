    ...
    const buildFileItem = async (
        ...
        if (file.size > MAX_FILE_SIZE) {
            return {
                ...
                name,
                type: "file",
                content: `File too large: ${name} (${Math.round(
                    file.size / 1024
                )}KB)`,
                isLazy: false,
                fileHandle: null,
    ...
    const readDirectory = async (
        ...
        for await (const entry of directoryHandle.values()) {
            if (entry.kind === "file") {
                ...
                const newFile = await buildFileItem(file, entry.name, entry)
    ...
    const readFileList = async (files: FileList): Promise<FileSystemItem[]> => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (pathParts.length > 1) {
                ...
                const newFile = await buildFileItem(file, file.name, file)
