...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const [fileLocks, setFileLocks] = useState<
        Record<string, { lockedBy?: string; lockedById?: string }>
    >({})
    const locksRef = useRef(fileLocks)

    useEffect(() => {
        locksRef.current = fileLocks
    }, [fileLocks])
    ...
    const isLockedByOthers = useCallback(() => {
        return Object.values(locksRef.current).some(
            (lock) => lock.lockedById && lock.lockedById !== currentUser.uid,
        )
    }, [currentUser.uid])

    const guardFileOps = useCallback(() => {
        if (!isLockedByOthers()) return true
        const firstLock = Object.values(locksRef.current)[0]
        const owner = firstLock?.lockedBy || "a teammate"
        toast.error(`File operations locked by ${owner}`)
        return false
    }, [isLockedByOthers])

    const stripFileHandles = useCallback((item: FileSystemItem): FileSystemItem => {
        if (item.type === "file") {
            return {
                ...item,
                fileHandle: null,
        ...
        return {
            ...item,
            children: item.children?.map(stripFileHandles),
        }
    }, [])
    ...
    const createDirectory = useCallback(
        (
            ...
            if (sendToSocket && !guardFileOps()) return ""
        ...
        [fileStructure.id, guardFileOps, socket],
    )
    ...
    const updateDirectory = useCallback(
        (
            ...
            const sanitizedChildren = children.map(stripFileHandles)
            socket.emit(SocketEvent.DIRECTORY_UPDATED, {
                dirId,
                children: sanitizedChildren,
            })
        },
        [fileStructure.id, guardFileOps, socket, stripFileHandles],
    )
    ...
    const renameDirectory = useCallback(
        (
            ...
            if (sendToSocket && !guardFileOps()) return false
        ...
        [fileStructure, guardFileOps, setFileStructure, socket],
    )
    ...
    const openFile = (fileId: Id) => {
        ...
        if (file) {
            ...
            if (file.type === "file" && file.isLazy && file.fileHandle) {
                loadLazyFile(file)
            }
        }
    }
    ...
    const createFile = useCallback(
        ...
        [fileStructure, guardFileOps, socket],
    )
    ...
    const readFileContentFromHandle = async (
        handle: FileSystemFileHandle | File,
        name: string,
    ) => {
        try {
            const file =
                "getFile" in handle ? await handle.getFile() : handle
    ...
    const loadLazyFile = useCallback(
    ...
    const renameFile = useCallback(
        ...
        [activeFile?.id, guardFileOps, socket],
    )
    ...
    const deleteFile = useCallback(
        ...
        [activeFile?.id, guardFileOps, openFiles, socket],
    )
    ...
    const handleUserJoined = useCallback(
        ({ user }: { user: RemoteUser }) => {
            ...
            const sanitizedStructure = stripFileHandles(fileStructure)
            const sanitizedOpenFiles = openFiles.map(stripFileHandles)
            const sanitizedActiveFile = activeFile
                ? stripFileHandles(activeFile)
                : null
            socket.emit(SocketEvent.SYNC_FILE_STRUCTURE, {
                fileStructure: sanitizedStructure,
    ...
    const handleFileLockStatus = useCallback(
        ({
            fileId,
            locked,
            lockedBy,
            ...
            if (locks) {
                const nextLocks: Record<
                    string,
                    { lockedBy?: string; lockedById?: string }
                > = {}
                locks.forEach((lock) => {
                    nextLocks[lock.fileId] = {
            ...
            setFileLocks((prev) => {
                const updated = { ...prev }
                if (locked) {
                    updated[fileId] = { lockedBy, lockedById }
                } else {
    ...
    const lockFile = useCallback(
        (fileId: string) => {
            if (!currentUser.roomId || !fileId) return
            socket.emit(SocketEvent.FILE_LOCK, { fileId })
        },
    ...
    const unlockFile = useCallback(
        (fileId: string) => {
            if (!currentUser.roomId || !fileId) return
            socket.emit(SocketEvent.FILE_UNLOCK, { fileId })
        },
        [currentUser.roomId, socket],
    )
    ...
    const getLockInfo = useCallback(
        (fileId?: string) => {
            ...
            return locksRef.current[fileId] || null
        },
        [],
    )
    ...
    useEffect(() => {
        ...
        socket.on(SocketEvent.FILE_LOCK_STATUS, handleFileLockStatus)
        ...
        return () => {
            ...
            socket.off(SocketEvent.FILE_LOCK_STATUS)
        }
    }, [
        handleDirCreated,
        handleDirDeleted,
